...
`pnpm run check` runs type checking, unit tests, production builds, an allowlist check over `npm pack --dry-run`, and creates `dist/dsh-codex-claude-cli-X.Y.Z.tgz`.
...
5. After the package exists, open its npm settings and configure the trusted publisher with:
   ...
   - Repository: `dsh-codex-claude-cli`
