...
/**
 * Prefix Codex reserves for the MCP servers it mounts itself.
 *
 * A dynamic tool declared under this prefix makes App Server reject the whole
 * `thread/start` with `dynamic tool name is reserved`, so the turn dies before
 * the model sees anything. Harness names every MCP tool
 * `mcp__<server>__<tool>`, which means one MCP server configured in Harness
 * takes the entire provider down.
 */
const CODEX_RESERVED_TOOL_PREFIX = 'mcp__'

/** What a Harness MCP tool is renamed to on the App Server side. */
const HARNESS_MCP_TOOL_PREFIX = 'harness_mcp__'

/**
 * The name one Harness tool is declared under inside the App Server namespace.
 *
 * Only two names are rewritten, and both round-trip: `harnessToolCall` rebuilds
 * the same mapping and looks the original up, and `bridgedTools` rejects a
 * collision, so a rename can never silently redirect a call.
 */
function appServerToolName(name: string): string {
  if (name === HARNESS_SKILL_NAME) return HARNESS_SKILL_TOOL_NAME
  if (name.startsWith(CODEX_RESERVED_TOOL_PREFIX)) {
    return HARNESS_MCP_TOOL_PREFIX + name.slice(CODEX_RESERVED_TOOL_PREFIX.length)
  }
  return name
}
